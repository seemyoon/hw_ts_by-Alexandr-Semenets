...
const array1: number[] = [132, 2132, 33, 44, 53353];
// - з 5 стічкових значень
const array2: string[] = ["Alexandr", "Ivan", "Max", "Andrew", "Petro"];
// - з 5 значень стрічкового, числового та булевого типу
const array3: unknown[] = [32, "Ivan", false, "game", 98];
...
const array5: number[] = [2, 17, 13, 6, 22, 31, 45, 66, 100, -18]
let i: number = 0;
...
for (let j: number = 0; j < array5.length; j++) {
...
for (let y: number = 0; y < array5.length; y++) {
    if (y % 2) {
...
for (let q: number = 0; q < array5.length; q++) {
    if (!(array5[q] % 2)) {
...
const newArray = [];
for (let n: number = array5.length - 1; n > 0; n--) {
...
// 1
let i2: number = array5.length - 1;
while (i2 >= 0) {
    console.log(array5[i2])
    i2--
}
// 2
for (let j2: number = array5.length - 1; j2 >= 0; j2--) {
    console.log(array5[j2])
}
// 3
let x2: number = array5.length - 1;
while (x2 > 0) {
    if (x2 % 2) {
...
// 4
for (let y: number = array5.length - 1; y > 0; y--) {
    if (y % 2) {
        console.log(array5[y])
    }
}
// 5
let z2: number = array5.length - 1;
while (z2 >= 0) {
    if (!(array5[z2] % 2)) {
...
// 6
for (let q2: number = array5.length - 1; q2 >= 0; q2--) {
    if (!(array5[q2] % 2)) {
...
const numbersArray: number[] = [42, 3.14, 100, -5, 0, 1.618, -273, 7.777, 123456789, -987654321];
...
for (let i: number = 0; i < 100; i = i + 2) {
    console.log(i)
    document.write(`${i} `)
}
document.write("<br>") // my additional
...
const books: { title: string, pageCount: number, authors: string[], genres: string[] }[] = [
    {
...
let theBiggestBook: { title: string, pageCount: number, authors: string[], genres: string[] } = books[0];
...
let genreResult: {}[] = [];
let theBiggestGenre: { title: string, pageCount: number, authors: string[], genres: string[] } = books[0];
for (const book of books) {
